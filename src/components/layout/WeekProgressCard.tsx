...
export function WeekProgressCard() {
  const now = new Date()
  const { step, total, dayIndex, phrase, fillRatio } = weekTowardFriday(now)
  const markerCenterPct = ((dayIndex + 0.5) / 7) * 100

  return (
    <div className="border-line border-t p-3 pb-4 dark:border-line-dark">
      <div className="mx-auto w-full max-w-full min-w-0 rounded-2xl border border-stone-200/90 bg-gradient-to-b from-white to-orange-50/30 px-2.5 pt-4 pb-4 shadow-md shadow-orange-200/25 dark:border-stone-600/80 dark:from-stone-900 dark:to-violet-950/40 dark:shadow-violet-950/20">
        <div className="mb-1 grid min-w-0 grid-cols-7">
          {DAYS.map((_, i) => (
            <div key={i} className="flex min-w-0 justify-center" aria-hidden>
              <div className="h-1.5 w-px bg-stone-200 dark:bg-stone-600" />
            </div>
          ))}
        </div>

        <div className="relative w-full min-w-0 pt-1 pb-0.5" role="presentation">
          <div
        ...
        <div className="mt-2 grid min-w-0 grid-cols-7 gap-0 text-center">
          {DAYS.map((label, i) => (
            <span
              key={label}
              className={`min-w-0 truncate text-[8px] font-bold leading-none tracking-tight text-ink-muted dark:text-stone-500 ${
